...
export EXT_FONT_DIR="/system-lib/qt/lib/fonts/"
...
echo "Original fb rotation is set @ ${ORIG_FB_ROTA}" >>/app-temp/crash.log 2>&1
...
echo "Original fb bitdepth is set @ ${ORIG_FB_BPP}bpp" >>/app-temp/crash.log 2>&1
# Sanity check...
case "${ORIG_FB_BPP}" in
    8) ;;
    16) ;;
...
ko_do_fbdepth() {
    ...
    if [ "${PLATFORM}" = "b300-ntx" ]; then
        ...
        echo "Making sure that rotation is set to Portrait" >>/app-temp/crash.log 2>&1
        /system-lib/lib/ld-linux-armhf.so.3 ./fbdepth -R UR >>/app-temp/crash.log 2>&1
    ...
    if grep -q '\["dev_startup_no_fbdepth"\] = true' 'settings.reader.lua' 2>/dev/null; then
        ...
        if [ -n "${ORIG_FB_BPP}" ]; then
            ...
            if [ "${ORIG_FB_BPP}" -eq "16" ] && [ "${PRODUCT}" != "frost" ] && [ "${PRODUCT}" != "storm" ]; then
                echo "Making sure we're using the original fb bitdepth @ ${ORIG_FB_BPP}bpp & rotation @ ${ORIG_FB_ROTA}" >>/app-temp/crash.log 2>&1
                /system-lib/lib/ld-linux-armhf.so.3 ./fbdepth -d "${ORIG_FB_BPP}" -r "${ORIG_FB_ROTA}" >>/app-temp/crash.log 2>&1
            else
                echo "Making sure we're using the original fb bitdepth @ ${ORIG_FB_BPP}bpp, and that rotation is set to Portrait" >>/app-temp/crash.log 2>&1
                /system-lib/lib/ld-linux-armhf.so.3 ./fbdepth -d "${ORIG_FB_BPP}" -R UR >>/app-temp/crash.log 2>&1
            fi
        fi
    else
        # Swap to 8bpp if things looke sane
        if [ -n "${ORIG_FB_BPP}" ]; then
            echo "Switching fb bitdepth to 8bpp & rotation to Portrait" >>/app-temp/crash.log 2>&1
            /system-lib/lib/ld-linux-armhf.so.3 ./fbdepth -d 8 -R UR >>/app-temp/crash.log 2>&1
        fi
    fi
}

# we keep at most 500KB worth of crash log
if [ -e /app-temp/crash.log ]; then
    tail -c 500000 /app-temp/crash.log >/app-temp/crash.log.new
    mv -f /app-temp/crash.log.new /app-temp/crash.log
fi
...
while [ ${RETURN_VALUE} -ne 0 ]; do
    ...
    ./reader.lua "$@" >>/app-temp/crash.log 2>&1
    RETURN_VALUE=$?
    ...
    if [ ${RETURN_VALUE} -ne 0 ] && [ ${RETURN_VALUE} -ne ${KO_RC_RESTART} ] && [ ${RETURN_VALUE} -ne ${KO_RC_USBMS} ] && [ ${RETURN_VALUE} -ne ${KO_RC_HALT} ]; then
        ...
        viewHeight=800
        FONTH=16
        eval "$(/system-lib/lib/ld-linux-armhf.so.3 ./fbink -e | tr ';' '\n' | grep -e viewWidth -e viewHeight -e FONTH | tr '\n' ';')"
        ...
        /system-lib/lib/ld-linux-armhf.so.3 ./fbink -q ${FBINK_BATCH_FLAG} -c -B GRAY9 -m -t regular=./fonts/freefont/FreeSerif.ttf,px=${bombHeight},top=${bombMargin} -W ${FBINK_WFM} -- $'\xf0\x9f\x92\xa3'
        # With a little notice at the top of the screen, on a big gray screen of death ;).
        # shellcheck disable=SC2086
        /system-lib/lib/ld-linux-armhf.so.3 ./fbink -q ${FBINK_BATCH_FLAG} ${FBINK_BGLESS_FLAG} -m -y 1 -W ${FBINK_WFM} -- "Don't Panic! (Crash n°${CRASH_COUNT} -> ${RETURN_VALUE})"
        if [ ${CRASH_COUNT} -eq 1 ]; then
            # Warn that we're waiting on a tap to continue...
            # shellcheck disable=SC2086
            /system-lib/lib/ld-linux-armhf.so.3 ./fbink -q ${FBINK_BATCH_FLAG} ${FBINK_BGLESS_FLAG} -m -y 2 -W ${FBINK_WFM} -- "Tap the screen to continue."
        fi
        # And then print the tail end of the log on the bottom of the screen...
        crashLog="$(tail -n 25 /app-temp/crash.log | sed -e 's/\t/    /g')"
        ...
        # shellcheck disable=SC2086
        /system-lib/lib/ld-linux-armhf.so.3 ./fbink -q ${FBINK_BATCH_FLAG} ${FBINK_BGLESS_FLAG} -t regular=./fonts/droid/DroidSansMono.ttf,top=$((viewHeight / 2 + FONTH * 2 + FONTH / 2)),left=$((viewWidth / 60)),right=$((viewWidth / 60)),px=$((viewHeight / 64))${FBINK_OT_PADDING} -W ${FBINK_WFM} -- "${crashLog}"
        if [ "${PLATFORM}" != "b300-ntx" ]; then
            ...
            /system-lib/lib/ld-linux-armhf.so.3 ./fbink -q -f -s
        fi
        ...
        } >>/app-temp/crash.log 2>&1
        if [ ${CRASH_COUNT} -lt 5 ] && [ "${ALWAYS_ABORT}" = "false" ]; then
            echo "Attempting to restart KOReader . . ." >>/app-temp/crash.log 2>&1
            echo "!!!!" >>/app-temp/crash.log 2>&1
        fi
        ...
        if [ ${CRASH_COUNT} -ge 5 ]; then
            echo "Too many consecutive crashes, aborting . . ." >>/app-temp/crash.log 2>&1
            echo "!!!! ! !!!!" >>/app-temp/crash.log 2>&1
            break
        fi
        ...
        if [ "${ALWAYS_ABORT}" = "true" ]; then
            echo "Aborting . . ." >>/app-temp/crash.log 2>&1
            echo "!!!! ! !!!!" >>/app-temp/crash.log 2>&1
            break
        fi
    else
...
if [ -n "${ORIG_FB_BPP}" ]; then
    echo "Restoring original fb bitdepth @ ${ORIG_FB_BPP}bpp & rotation @ ${ORIG_FB_ROTA}" >>/app-temp/crash.log 2>&1
    /system-lib/lib/ld-linux-armhf.so.3 ./fbdepth -d "${ORIG_FB_BPP}" -r "${ORIG_FB_ROTA}" >>/app-temp/crash.log 2>&1
else
    echo "Restoring original fb rotation @ ${ORIG_FB_ROTA}" >>/app-temp/crash.log 2>&1
    /system-lib/lib/ld-linux-armhf.so.3 ./fbdepth -r "${ORIG_FB_ROTA}" >>/app-temp/crash.log 2>&1
fi
